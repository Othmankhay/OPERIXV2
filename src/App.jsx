...
/* ─── Export page ────────────────────────────────────── */
const _EXPORT_STATUTS = ["Reçu","Point dur","Manquants Plus","Manquant","En cours","À venir","Faux manquant","Retard","Confirmé","Archivé"];
const _EXPORT_WEEKS   = ["S18","S19","S20","S21","S22","S23"];
const _EXPORT_STATUT_CONFIG = {
  ...STATUT_CONFIG,
...
function PageExport({ filteredCount, totalCount, filteredData, allData }) {
  ...
  const exportChartPng = (data) => {
    if (!data || data.length === 0) return;

    // Rebuild weekly data from raw rows (same logic as PageGraphique)
    const weekData = {};
    _EXPORT_STATUTS.forEach(st => { weekData[st] = [0,0,0,0,0,0]; });
    data.forEach(row => {
      ...
      if (weekIdx >= 0 && weekIdx < 6 && row.statut && _EXPORT_STATUTS.includes(row.statut)) {
        weekData[row.statut][weekIdx]++;
      }
    });

    const curvePtsData = _EXPORT_WEEKS.map((_, i) => _EXPORT_STATUTS.reduce((s, st) => s + (weekData[st]?.[i] || 0), 0));
    const maxVal = Math.max(...curvePtsData, 10) * 1.15;
    const yLabels = [1, 0.75, 0.5, 0.25, 0].map(r => Math.round(maxVal * r));

    const svgW = 880;
    const marginLeft = 56, marginTop = 54, chartW = 780, chartH = 270;
    const barGroupW = chartW / _EXPORT_WEEKS.length;
    const barW = barGroupW * 0.52;

    let bars = '';
    _EXPORT_WEEKS.forEach((w, wi) => {
      const barX = marginLeft + wi * barGroupW + (barGroupW - barW) / 2;
      let yBase = marginTop + chartH;
      _EXPORT_STATUTS.forEach(st => {
        const val = weekData[st][wi];
        if (!val) return;
        const h = (val / (maxVal || 1)) * chartH;
        const sc = _EXPORT_STATUT_CONFIG[st];
    ...
    const pts = _EXPORT_WEEKS.map((_, wi) => ({
      x: marginLeft + (wi + 0.5) * barGroupW,
      y: marginTop + chartH - (curvePtsData[wi] / (maxVal || 1)) * chartH,
    }));
    let pathD = '';
    pts.forEach((pt, wi) => {
    ...
    const legendItems = _EXPORT_STATUTS.map((st, idx) => {
      const sc = _EXPORT_STATUT_CONFIG[st];
    ...
    const legendRows = Math.ceil(_EXPORT_STATUTS.length / colCount);
...
<text x="${svgW - 10}" y="32" font-size="11" fill="#94a3b8" text-anchor="end">Export du ${dateStr}</text>
          ...
          <div key={c.title} style={{ background: "#fff", borderRadius: 14, padding: 20, border: "1px solid #e2e8f0" }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {c.isGraph ? (
                <>
                  <button
                    style={{ flex: 1, padding: "8px 14px", borderRadius: 8, border: "1px solid #fbcfe8", background: "#fdf2f8", cursor: "pointer", fontWeight: 600, fontSize: 13, color: "#be185d", transition: "all 0.2s" }}
